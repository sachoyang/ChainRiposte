...
**구조 구현됨 (2026-07-21)** — 배치 값은 씬에서 편집한다.
- `OrientationService`: 해상도로 방향을 판정해 알리는 허브 (씬 배치 불필요, 자동 부팅).
- `OrientationLayout`: UI 요소마다 **세로/가로 프리셋 2벌**(앵커·피벗·위치·크기·스케일)을 갖고 방향 전환 시 적용. 씬에서 드래그해 배치한 뒤 인스펙터 버튼으로 저장 = 코드 0줄.
- `OrientationCanvas`: 기준 해상도를 1080×1920 ↔ 1920×1080으로 전환.
- `CameraFit2D`: 화면 크기가 바뀌면 마지막 영역으로 자동 재조정 (보드/월드맵).
- 기본 가로 배치는 빌더가 깔아둔다 — 전투 2버튼 = 양쪽 사이드, 스탯 분배 버튼 = 우측 세로 스택, 월드맵 정보 패널 = 우측 컬럼.
