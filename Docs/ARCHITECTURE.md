...
| 7 | 전투 코어 (2버튼, 패턴 실행기, 체간, 인살) — `Core/Combat/`, BossDataSO | ✅ 완료 |
| 8 | 전투 프레젠테이션 + 결과 화면 — `Game/Combat/`, ResultScreen(씬 리로드 재시작) | ✅ 완료 |
| 9 | Game Juice 훅 정비 — `Game/Juice/` JuiceDirector(클립 슬롯 허브)+셰이크/히트스톱/PostFx | ✅ 완료 |
| 10 | 스테이지 선택 씬 — NSMB식 월드맵 (6스테이지, 클릭 이동 + 정보 패널, GDD §9) — `Game/Map/`, StageSelection 정적 홀더 | ✅ 완료 |
| 확장 | 스테이지 기믹 3종 — 전염되는 타일 / 시한폭탄 몬스터 / 사슬 결박 (GDD §3.6) — 월드 2에 적용 | ⬜ |
| 확장 | 월드맵 진행도 잠금 + 세이브 (GDD §9.2) | ⬜ |
| 확장 | 월드 2 콘텐츠 — 전용 배경/보스/기믹 조합 (GDD §9.2) | ⬜ |
| 확장 | 모바일 세로 뷰 대응 — 방향별 UI 재배치 구조 (GDD §9.3, 출시 요구사항) | ⬜ |
| 확장 | StageDataSO 씬뷰 프리뷰 — 보드 마스킹/벽 배치를 에디터에서 시각화 (기즈모 또는 커스텀 에디터, GDD §3.2 툴링과 연계) | ⬜ |
