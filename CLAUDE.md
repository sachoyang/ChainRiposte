...
- 🔴 **`Boss_03.asset`은 이제 아무도 참조하지 않는다** — 지울지 사용자에게 확인할 것(죽은 에셋을 남기는 것도 함정).
...
3. `Boss_03.asset` 삭제 여부.
4. 그 다음: **Boss_02 채보**(미리듣기 툴 있음) 또는 **난이도 커브 체감 튜닝**(세션 13 몫) 또는 **사운드 클립**(오디오 0개).

#### ✅ 세션 14 검증 부탁 (전부 미검증)

1. **기억 획득** — 1-1을 깨면 결과 화면에 `새 기억 — 칼끝의 기억`이 뜨는지. 1-2를 또 깨면 **안 뜨는지**(같은 보스).
2. **아이콘 줄** — 퍼즐 HUD 좌상단 / 준비 화면 띠 / 결과 화면에 아이콘이 뜨는지. 위치는 씬에서 드래그해 맞출 것.
3. **보호막** — 2-1을 깬 뒤(기억 `memory_ward` 획득) 다음 판에서 **연속 3패링 → 다음 피격이 청백색 "기억이 막았다"로 무효**가 되는지.
