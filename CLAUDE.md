...
- **테스트**: EditMode **139/139** (MCP 확인). Core 건드리면 초록 유지.
...
1. **난이도 체감 확인** — 실제로 1-1과 2-3을 쳐 보고 커브 수치를 조정한다.
   다이얼은 `DifficultyCurve.asset` 하나. 물렁하면 `고리당 체간` → `고리당 피해` → `고리당 템포` 순으로 올린다.
   특정 보스만 험하게 = **그 보스 에셋**(커브를 만지면 6판이 같이 움직인다).
2. **`Stage_01.asset`의 `bossEngageSeconds`가 15로 남아 있다** (커밋 `9f7628f` "시험용"). 원래 90.
   Main 단독 실행용 스테이지라 게임 흐름에는 안 쓰이지만, 시험이 끝났으면 되돌릴 것.
3. 그 다음은 로드맵 **4번 넋 시스템**(`SoulRelicSO` — `RunState.AcquiredRelicIds` 자리는 이미 있다)
   또는 **Boss_02 채보**(미리듣기 툴이 생겨 병목이 풀렸다) 또는 **사운드 클립**(오디오 파일 여전히 0개).

#### 🟠 별건으로 남긴 것 — Boss_02와 Boss_03이 같은 보스인데 에셋이 둘

페이즈 때문에 나눠 놓은 것이라, **그 보스의 채보·수치를 고치려면 두 곳에 똑같이 적어야 한다**
(세션 12의 `portrait`/`battleSprite` 중복과 같은 함정). "2페이즈로 싸울지는 스테이지가 정한다"로
합치는 게 자연스럽지만 데이터 모델을 건드려야 해서 이번 작업과 분리했다.
