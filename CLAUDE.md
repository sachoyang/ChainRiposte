...
**⑤ 성난 몬스터 — 종류별 공격 주기 (사용자 요청)**
- `TileDefinitionSO ▸ 공격까지 초` 신규(0이면 공용값). 피해(`attackDamage`)는 전부터 있었다 —
  **타일 6종 모두 0이라 지금은 전부 공용 8 피해**다.
- 카운트다운은 **초가 아니라 박**으로 돈다(모두 같은 맥박). 그래서 초는 성날 때
  `반올림(초 ÷ 박 길이)`로 박이 되고 **최소 1박**이다(0박이면 예고 없이 때린다).
  박 길이 1.6초에서 3초 → 2박(3.2초). **초를 그대로 쓰려면 스테이지의 `박 길이`를 1초로.**
- ⚠ 지금 공용값은 3박 × 1.6초 = **4.8초**다("3초"가 아니다 — 3은 박 수).
- 퍼즐 피해는 **DEF를 안 탄다**(`Health.ApplyDamage(raw)`). 전투 피해만 `damage − DamageReduction`이다.
  의도인지 확인 필요 — 방어를 올려도 퍼즐에서는 똑같이 아프다.

#### 🔴 다음 세션에서 가장 먼저 할 것

1. **기억 플레이 검증** (아래 「검증 부탁」). 특히 보호막·아이콘 줄.
2. **현지화 키 6개를 구글 시트에 넣기** — `Localization.csv`에 직접 적었으므로 **다음 시트 굽기에 날아간다.**
   `combat.popup.memoryguard` / `result.memory.gained` / `memory.edge.name`·`.desc` / `memory.ward.name`·`.desc`.
3. **퍼즐 피해가 DEF를 안 타는 것**이 의도인지 사용자에게 확인(위 ⑤).
