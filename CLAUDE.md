...
- 지도 노드 6개 → **`panel`**(9슬라이스). 노드 트랜스폼이 18.4×19로 부풀려져 있어서 테두리가 뭉개졌다
  → **스케일을 1로 되돌리고 `SpriteRenderer.size`로 크기를 준다**(2.94×3.04 = 예전과 같은 크기).
  라벨·배지는 월드 위치·크기를 그대로 복원해 화면은 안 변한다.
