...
- **진행 상황**: 로드맵 1~10단계 + A1/A2/B1/낙하버그 + **B2 진행도 잠금·세이브** + **B3 스테이지 기믹 3종** + **B4 세로/가로 뷰 구조** 완료. 백로그 A·B 소진.
- **사용자 확인 대기**: 세션 3-4(보스 하강/스프라이트) + B2 + B3 + B4 — 사용자가 "나중에 한 번에 검증" 하기로 함.
  → **체크리스트는 `Docs/VERIFICATION.md`에 정리됨. 검증 얘기가 나오면 이 파일부터 볼 것.**
- **git**: B4까지 커밋 완료. 원격 `origin`(GitHub) 있고 **로컬이 4커밋 앞섬 — push는 아직 안 함**.
- **커밋 규칙**: 메시지에 `Co-Authored-By: Claude` 트레일러를 넣지 않는다 (2026-07-21 지시, 기존 커밋도 전부 제거함).
